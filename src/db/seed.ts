...
import { users, habbits, entries, tags, habbitTags } from './schema.ts'
...
async function seed() {
  ...
  try {
    ...
    await db.delete(habbitTags) // Delete junction table
    await db.delete(habbits) // Delete habits
    ...
    await db
      .insert(habbitTags)
      .values([{ habbitId: exerciseHabit.id, tagId: healthTag.id }])
    ...
    console.log('\n📊 Seed Summary:')
    console.log(`- Demo user has ${userWithHabits?.habbits.length || 0} habits`)
    console.log('\n🔑 Login Credentials:')
